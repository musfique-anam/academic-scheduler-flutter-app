// lib/data/services/workload_validator_service.dart
...
class WorkloadValidatorService {
  ...
  List<WorkloadInfo> validateTeacherWorkloads(
    ...
    for (var teacher in teachers) {
      ...
      var teacherClasses = routines.where((r) => r.teacherId == teacher.id.toString()).toList();
  ...
  Map<int, Map<String, dynamic>> validateBatchCredits(
    ...
    for (var batch in batches) {
      ...
      var batchClasses = routines.where((r) => r.batchId == batchId.toString()).toList();

      // Group by course
      Map<int, int> courseCount = {};
      for (var routine in batchClasses) {
        if (routine.courseId != null) {
  ...
  List<Teacher> getAvailableTeachers(
    ...
    t.id != null &&
        !busyTeacherIds.contains(t.id.toString()) &&
